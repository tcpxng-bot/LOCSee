...
function renderYear(slotDayMap, holidayMap, query = "") {
  ...
  MONTHS.forEach((month, index) => {
    ...
    for (let day = 1; day <= 31; day++) {
      ...
      const label = winner.length ? displayNames(winner) : daySlots.length ? `จอง ${candidateCount(daySlots[0].id)} คน` : holidays.length ? `หยุด ${displayNames(holidays.map(h => h.name))}` : "";
